                                                           ...
                                                           FGameplayTag ActivationTag)
{
	...
	if (ShouldBroadcastAbilityTaskDelegates())
	{
		...
		const FGameplayAbilityTargetData *Data = DataHandle.Get(0); // This will valid check the index for you 

		if (Data->GetScriptStruct() == FGameplayAbilityTargetData_SingleTargetHit::StaticStruct())
		{
			const FGameplayAbilityTargetData_SingleTargetHit *HitInfoData = static_cast<const
				FGameplayAbilityTargetData_SingleTargetHit *>(Data);
			UE_LOG(LogTemp, Warning, TEXT("Hit Distance %.2f"), HitInfoData->HitResult.Distance);
		}
